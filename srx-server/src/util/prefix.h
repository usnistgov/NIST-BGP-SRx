...
#define GET_AF_OF_IP(IP_ADDR) \
  (((IP_ADDR).version == 4) ? AF_INET : AF_INET6)
...
#define GET_SIZEOF_IP_VERSION(VER) \
  (((VER) == 4) ? sizeof(IPv4Address) : sizeof(IPv6Address))
...
#define GET_MAX_PREFIX_LEN(IP_ADDR) \
  (((IP_ADDR).version == 4) ? MAX_PREFIX_LEN_V4 : MAX_PREFIX_LEN_v6)
 ...
 * 
 * @return \c false = parsing failed - invalid format
 */
