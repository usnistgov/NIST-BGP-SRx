    ...
    if (cfgHlp && params->errMsgBuff[0] == '\0')
    {
      ...
      {        
        ...
        for (sessIdx = 0; sessIdx < sessCt; sessIdx++)
        {
          ...
          bgpConf = &sParam->bgpConf;
#else
          // With multi session this will be an array / Or list or so.
          bgpConf = &sParam->bgpConf[sessIdx];
#endif
  ...
  while (idx < argc && params->errMsgBuff[0] == '\0')
  {
    ...
    switch (getShortParam(argv[idx]))
    {
      ...
      case P_C_BGP_IDENT:
        ...
        params->bgpConf.bgpIdentifier = ipAddr.sin_addr.s_addr;
        break;
      ...
      case P_C_NEXT_HOP_IPV4:
        ...
        _setIPAddress(argv[idx], 0, &params->bgpConf.nextHopV4);
        break;
        
      case P_C_NEXT_HOP_IPV6:
        if (++idx >= argc) 
          { _setErrMsg(params, "Next Hop IPv6 not specified!"); break; }
        _setIP6Address(argv[idx], 0, &params->bgpConf.nextHopV6);
        break;
