...
# Date: Fri Mar 20 11:48:08 EDT 2020
# CMD: /usr/local/bin/get-BGPSRx -s

SCRIPTNAME='BGP-SRx Software Suite V3.0.1 Build script!'
...
LOC_PREFIX=$(echo $INSTALL_DIR/local-3.0.1 | sed -e "s/\/\.\//\//g")
...
{
  echo
  echo "Syntax: $0 [SRxSnP] [QSRx] [-C|-M|-I|-D|-U]"
  echo
...
if [ $DO_SRxSnP -eq 1 ] ; then
  ...
  if [ $DO_CONFIGURE -eq 1 ] ; then
    ...
    checkConfig src
    ./configure --prefix=$LOC_PREFIX --with-libconf --with-libncur --with-libpatr CFLAGS="-O0 -g"
...
if [ $DO_QSRx -eq 1 ] ; then
  ...
  if [ $DO_CONFIGURE -eq 1 ] ; then
    # Check if a configure script exist, otherwise try to generate one!
    checkConfig
    ./configure --prefix=$LOC_PREFIX --disable-doc --enable-user=root --enable-group=root --enable-srx SRX_USER_INC=$LOC_PREFIX/include CPPFLAGS=-I$LOC_PREFIX/include CFLAGS="-O0 -g"
    check $? "Error Configuring $NAME!"
  fi
  make_install $NAME
fi
