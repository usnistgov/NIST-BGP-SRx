#!/bin/bash
# This script is auto-generated by BGP-SRx Software Suite Bundler V4.6

# Date: Mon Jun 21 15:27:58 EDT 2021
# CMD: /bin/get-BGPSRx -S -i BGP-SRx-Latest --latest

SCRIPTNAME='BGP-SRx Software Suite V5.1.5 Build script V4.6!'
...
LOC_PREFIX=$(echo $INSTALL_DIR/local-5.1.5 | sed -e "s/\/\.\//\//g")

# This is used for the .vscode json files.
REL_LOC_PREFIX="local-5.1.5"
...
if [ $DO_EXAMPLES -eq 1 ] ; then
  ...
  cd $DIR
  if [ $DO_CONFIGURE -eq 1 ] ; then
    ./configure.sh --no-interactive Y AUTO
    check $? "Error Configuring $NAME!"
  fi
  if [ $DO_INSTALL -eq 1 ] ; then
